...
public class PlotClustering extends Frame {
	...
	private static final Logger log = LoggerFactory
			.getLogger(PlotClustering.class);
	...
	public void paint(Graphics g) {
		...
		log.info("Calling plotSampleData ");
		plotSampleData(g2);
		//plotSampleParameters(g2);
		log.info("Calling plotClusters ");
		plotClusters(g2);
	}

	protected static void plotClusters(Graphics2D g2) {
		int cx = CLUSTERS.size() - 1;
		log.info("plotClusters CLUSTERS.size() : " + CLUSTERS.size());
		for (List<Cluster> clusters : CLUSTERS) {
			
			g2.setStroke(new BasicStroke(cx == 0 ? 3 : 1));
			g2.setColor(COLORS[Math.min(PlotClustering.COLORS.length - 1,
					cx--)]);
			for (Cluster cluster : clusters) {
				log.info("plotClusters vector : " + cluster.getCenter().toString());
	...
	protected static void plotSampleData(Graphics2D g2) {
		...
		plotRectangle(g2, new DenseVector(11).assign(11), dv);
		plotRectangle(g2, new DenseVector(11).assign(11), dv);
		initRectangele = false;
		// plot the sample data
		g2.setColor(Color.DARK_GRAY);
		dv.assign(0.03);
		log.info("plotSampleData SAMPLE_DATA.size() : " + SAMPLE_DATA.size());
		for (VectorWritable v : SAMPLE_DATA) {
			log.info("plotSampleData vector : " + v.get().toString());
			try{
	...
	private static boolean initRectangele = true;
	/**
	...
	protected static void plotRectangle(Graphics2D g2, Vector v, Vector dv) {
		log.info("plotRectangle v :" +v.toString());
		
		double[] flip = { 1, 0.9, 0.8, 0.7, 0.6, 0.5, -0.6, -0.7, -0.8, -0.9, -1};
		Vector v2 = v.times(new DenseVector(flip));
		/*if(initRectangele){
			v2 = v.times(new DenseVector(flip0));
		}else{
			v2 = 
		}*/
		//v2 = v2.minus(dv.divide(11));
		int h = SIZE / 2;
		double x = v.get(0) + h;
		double y = v.get(1) + h;
		log.info("plotRectangle : " + v.toString() + " X: " +x+ " Y:" +y);
	...
	protected static void plotEllipse(Graphics2D g2, Vector v, Vector dv) {
		log.info("plotEllipse v.size() : " + v.size());
		double[] flip = { 1, 0.9, 0.8, 0.7, 0.6, 0.5, -0.6, -0.7, -0.8, -0.9, -1};
		Vector v2 = v.times(new DenseVector(flip));
		//v2 = v2.minus(dv.divide(11));
		int h = SIZE / 2;
		double x = v.get(0) + h;
		double y = v.get(1) + h;
		log.info("plotEllipse : " + v.toString() + " X: " +x+ " Y:" +y);
		g2.draw(new Ellipse2D.Double(x * DS, y * DS, dv.get(0) * DS, dv.get(1)
				* DS));
	}

	/*
	 * protected static void generateSamples() { generateSamples(500, 1, 1, 3);
	 * generateSamples(300, 1, 0, 0.5); generateSamples(300, 0, 2, 0.1); }
	 * 
	 * protected static void generate2dSamples() { generate2dSamples(500, 1, 1,
	 * 3, 1); generate2dSamples(300, 1, 0, 0.5, 1); generate2dSamples(300, 0, 2,
	 * 0.1, 0.5); }
	 */
	 ...
	 * double sd) { double[] params = {mx, my, sd, sd}; SAMPLE_PARAMS.add(new
	 * DenseVector(params)); log.info("Generating {} samples m=[{}, {}] sd={}",
	 * new Object[] {num, mx, my, sd}); for (int i = 0; i < num; i++) {
	 * SAMPLE_DATA.add(new VectorWritable(new DenseVector( new double[]
	 * {UncommonDistributions.rNorm(mx, sd), UncommonDistributions.rNorm(my,
	 * sd)}))); } }
	 * 
	 * protected static void writeSampleData(Path output) throws IOException {
	 * Configuration conf = new Configuration(); FileSystem fs =
	 * FileSystem.get(output.toUri(), conf); SequenceFile.Writer writer = new
	 * SequenceFile.Writer(fs, conf, output, Text.class, VectorWritable.class);
	 * try { for (VectorWritable vw : SAMPLE_DATA) { writer.append(new Text(),
	 * vw); } } finally { Closeables.closeQuietly(writer); } }
	 */
	...
	protected static void loadSamples(Path samples) throws IOException {
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(conf);
		SequenceFile.Reader reader = new SequenceFile.Reader(fs, samples, conf);
		LongWritable key = new LongWritable();
		VectorWritable value = new VectorWritable();
		while (reader.next(key, value)) {
			SAMPLE_DATA.add(value);
		}		
	}
	 ...
	 * double sdx, double sdy) { double[] params = {mx, my, sdx, sdy};
	 * SAMPLE_PARAMS.add(new DenseVector(params));
	 * log.info("Generating {} samples m=[{}, {}] sd=[{}, {}]", new Object[]
	 * {num, mx, my, sdx, sdy}); for (int i = 0; i < num; i++) {
	 * SAMPLE_DATA.add(new VectorWritable(new DenseVector( new double[]
	 * {UncommonDistributions.rNorm(mx, sdx), UncommonDistributions.rNorm(my,
	 * sdy)}))); } }
	 */
	...
	protected static ClusterClassifier readClassifier(Configuration config,
	...
	protected static void writeClassifier(ClusterClassifier classifier,
			Configuration config, Path path) throws IOException {
